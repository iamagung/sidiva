...
class LayananTelemedicineController extends Controller
{
    ...
    public function main(Request $request)
    {
        if(request()->ajax()){
            // return 'return';
            $data = LayananTelemedicine::orderBy('id_layanan_telemedicine','DESC')->get();
			return DataTables::of($data)
				->addIndexColumn()
				->addColumn('actions', function($row){
					$txt = "
                      <button class='btn btn-sm btn-primary' title='Edit' onclick='formAdd(`$row->id_layanan_telemedicine`)'><i class='fadeIn animated bx bxs-file' aria-hidden='true'></i></button>
                      <button class='btn btn-sm btn-danger' title='Delete' onclick='hapus(`$row->id_layanan_telemedicine`)'><i class='fadeIn animated bx bxs-trash' aria-hidden='true'></i></button>
					";
					return $txt;
				})
				->rawColumns(['actions'])
        ...
        return view('admin.telemedicine.layanan.main', $data);
    }
    ...
    public function form(Request $request)
    {
        if (empty($request->id)) {
			...
			$data['layanan'] = LayananTelemedicine::where('id_layanan_telemedicine', $request->id)->first();
            $data['title'] = "Edit ".$this->title;
		}
        $content = view('admin.telemedicine.layanan.form', $data)->render();
    ...
    public function store(Request $request)
    {
        ...
        try {
            if (empty($request->id)) {
                $data = new LayananTelemedicine;
            } else {
                $data = LayananTelemedicine::where('id_layanan_telemedicine', $request->id)->first();
            }
